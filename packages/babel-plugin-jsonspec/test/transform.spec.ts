...
type Check = (value: unknown) => Result<
  unknown,
  {
...
const generateCheck = <S extends Spec>(...specs: readonly [S, ...(readonly S[])]) => {
  try {
    let check = prefix
    ...
    const jsonspecImport = defScope.generateUidIdentifier('js').name
    ...
    console.log(inspect(specs, inspectConfig))
    throw e
  }
}
...
describe('transform - success', () => {
  ...
  test('tupleArray([string], filter(number, isEven))', () => {
    const check = generateCheck(['tupleArray', [['string']], ['number'], ['identifier', 'isEven']])
  ...
  test('lazy', () => {
    const check = generateCheck([
      ...
      [
        'object',
        { id: ['string'], items: ['array', ['validator', ['external', ['identifier', 'check0']]]] }
      ]
    ])
  ...
  test('struct({ x: tuple(optional(map(number, x => x + 1))) })', () => {
    ...
    fc.assert(
      fc.property(fc.record({ x: fc.tuple(fc.option(fc.float(), { nil: undefined })) }), (val) => {
        const {
          x: [v]
        } = val
