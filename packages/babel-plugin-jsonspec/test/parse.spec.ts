...
const fixtureFailureTest = (fixture: string, expected: string) =>
  fixtureTest(fixture, Result.failure(`jsonspec error: ${expected}`))
...
describe('parse', () => {
  ...
  astFailureTest(
    ...
    "jsonspec error: the 'struct' spec first argument property value that starts at ln:NaN col:NaN, ends at ln:NaN col:NaN should be an expression"
  )
  ...
  test('parseTemplate failure', () => {
    expect(
      ...
      failure:
        "jsonspec error: the 'template' spec that starts at ln:NaN col:NaN, ends at ln:NaN col:NaN has unsupported arguments",
      tag: 'failure'
    })
  })
})
